...
export const UIProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Quality Settings
  const [graphicsQuality, setGraphicsQualityState] = useState<
    "low" | "mid" | "high"
  >(() => {
    const saved = localStorage.getItem("tgs_settings_quality") as
      | "low"
      | "mid"
      | "high";
    return saved || "high";
  ...
  // Split Stack State
  const [splitStackState, setSplitStackState] = useState<{
    item: any;
    max: number;
    onConfirm: (count: number) => void;
  } | null>(null);

  const openSplitStack = (
    item: any,
    max: number,
    onConfirm: (count: number) => void,
  ) => {
  ...
  const [debugCollision, setDebugCollision] = useState(
  ...
  const setScale = (val: number) => {
    console.warn("Manual setScale ignored");
  };
  ...
  useEffect(() => {
    ...
    const handleRequestItemTooltip = (itemData: any) => {
      const { name, subtext: formattedSubtext } = formatItemTooltip(
        (itemData as any).def,
        itemData,
      );
      setTooltip({
    ...
    <UIContext.Provider
      value={{
        ...
        toggleDebugCollision: () =>
          PlayerState.getInstance().toggleDebugCollision(),
        graphicsQuality,
        setGraphicsQuality,
        showFPS,
        toggleFPS,
      ...
      {tooltip && (
        <div
          style={{
            ...
            bottom: isBottomSide
              ? window.innerHeight - tooltip.y + 15
              : undefined,
