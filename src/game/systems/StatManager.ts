...
import { WeaponRegistry, WeaponDefinition } from "../entities/weapons/WeaponRegistry";
...
export class StatManager {
     ...
     */
    public calculateStat(statName: string, state: PlayerState, overrides?: Map<EquipmentSlot, any>): StatResult {
        const modifiers: StatModifier[] = this.gatherModifiers(statName, state, overrides);
        ...
        const flatModifiers = modifiers.filter(m => m.type === "FLAT" && m.category !== "base");
        ...
        if (wpPercent > 0) {
             if (["maxHealth", "maxMana", "attack", "defense", "armor", "speed", "capacity", "memory"].includes(statName)) {
    ...
    public getEquippedStarCounts(state: PlayerState): { gold: number, silver: number, bronze: number } {
        let gold = 0;
        let silver = 0;
        let bronze = 0;
        const slots: EquipmentSlot[] = [
            EquipmentSlot.HEAD, EquipmentSlot.BODY, EquipmentSlot.LEGS, EquipmentSlot.BOOTS, 
            EquipmentSlot.MAIN_HAND, EquipmentSlot.OFF_HAND, EquipmentSlot.RING, EquipmentSlot.NECK
        ];
     ...
     */
    private gatherModifiers(statName: string, state: PlayerState, overrides?: Map<EquipmentSlot, any>): StatModifier[] {
        const modifiers: StatModifier[] = [];

        // A. Base Stats (Level/Skill based)
        let baseVal = 0;
        switch (statName) {
            case "strength": baseVal = state.getBaseStrengthLevel(); break;
            case "dexterity": baseVal = state.getBaseDexterityLevel(); break;
            case "reflex": baseVal = state.getBaseReflexLevel(); break;
            case "intelligence": baseVal = state.getBaseIntelligenceLevel(); break;
            case "level": baseVal = state.getLevel(); break;
            case "maxHealth": baseVal = 100; break;
            case "maxMana": baseVal = 100; break;
            case "criticalChance": baseVal = 0; break; 
            case "criticalDamage": baseVal = 0; break; // Base 0% (1.0x Multiplier)
            case "speed": baseVal = 200; break; // Base Speed
            
            case "range": baseVal = 0; break; // Weapon Dependent
            case "cooldown": baseVal = 0; break; // Weapon is added as modifier in gatherModifiers
            case "lightRadius": baseVal = 0; break;
            case "expPerHit": baseVal = 0; break;
            case "fireResist": baseVal = 0; break;
            case "iceResist": baseVal = 0; break;
            case "poisonResist": baseVal = 0; break;
            case "energyResist": baseVal = 0; break;
            case "physicalResist": baseVal = 0; break;
            case "expDamagePercent": baseVal = 0; break; // New Stat


            case "attack": baseVal = 0; break; // Strict: Weapon is Modifier
            case "defense": baseVal = 0; break; // Strict: Shield is Modifier
            case "armor": baseVal = 0; break; // Strict: Equipment is Modifier
            
            case "memory": baseVal = state.baseMemory; break;
            case "capacity": baseVal = 400; break;
        }
        ...
        if (statName === "attack") {
            ...
            const weaponItem = overrides?.get(EquipmentSlot.MAIN_HAND) || state.getEquippedItemInSlot(EquipmentSlot.MAIN_HAND);
            const weapon = weaponItem ? WeaponRegistry.getWeaponDefinition(weaponItem.itemId) : null;
            let skillVal = 0;
            let sourceName = "Strength";
            
            if (!weapon || weapon.type === ItemType.MELEE || weapon.type === ItemType.SWORD || weapon.type === ItemType.AXE || weapon.type === ItemType.CLUB) {
                skillVal = state.getStrengthLevel();
                sourceName = "Strength";
            } else if (weapon.type === ItemType.RANGED || weapon.type === ItemType.DISTANCE) {
        ...
        slots.forEach(slot => {
            const equippedItem = overrides?.has(slot) ? overrides.get(slot) : state.getEquippedItemInSlot(slot);
            if (equippedItem) {
                ...
                if (def) {
                    // Boots Speed
                    if (statName === "speed" && def.type === ItemType.BOOTS && def.speedBonus) {
                         modifiers.push({ source: translatedName, attr: "speed", type: "FLAT", value: def.speedBonus, category: "equipment" });
                    }

                    // Defense (Shields/Weapons) -> "defense"
                    if (statName === "defense") {
                         if ((def.type === ItemType.SHIELD || def.type === ItemType.MELEE || def.type === ItemType.RANGED || def.type === ItemType.SWORD || def.type === ItemType.AXE || def.type === ItemType.CLUB) && def.defense) {
                             modifiers.push({ source: translatedName, attr: "defense", type: "FLAT", value: def.defense, category: "equipment" });
                         }
                    }
                    ...
                    if (statName === "armor") {
                         // Strict Body Armor
                         if ([ItemType.BODY_ARMOR, ItemType.LEGS, ItemType.HELMET, ItemType.BOOTS, ItemType.RING, ItemType.AMULET].includes(def.type)) {
                              if (def.armor) {
                                  modifiers.push({ source: translatedName, attr: "armor", type: "FLAT", value: def.armor, category: "equipment" });
                              }
                              // Fallback: If 'defense' is used as armor value in JSON for body items
                              else if (def.defense) {
                                  modifiers.push({ source: translatedName, attr: "armor", type: "FLAT", value: def.defense, category: "equipment" });
                              }
                         }
                    }
                    ...
                    if (statName === "range" && def.range) {
                         modifiers.push({ source: translatedName, attr: "range", type: "FLAT", value: def.range, category: "equipment" });
                    }
                    
                    // Cooldown
                    if (statName === "cooldown" && def.cooldown) {
                         modifiers.push({ source: translatedName, attr: "cooldown", type: "FLAT", value: def.cooldown, category: "equipment" });
                    }
                    
                    // Light Radius
                    if (statName === "lightRadius" && def.lightRadius) {
                         modifiers.push({ source: translatedName, attr: "lightRadius", type: "FLAT", value: def.lightRadius, category: "equipment" });
                    }
                    
                    // Exp Per Hit
                    if (statName === "expPerHit" && def.exp_skill) {
                         modifiers.push({ source: translatedName, attr: "expPerHit", type: "FLAT", value: def.exp_skill, category: "equipment" });
                    }
                    
                    // Resistances
                    if (def.resistances) {
                        if (statName === "fireResist" && def.resistances.fire) modifiers.push({ source: translatedName, attr: "fireResist", type: "FLAT", value: def.resistances.fire * 100, category: "equipment" });
                        if (statName === "iceResist" && def.resistances.ice) modifiers.push({ source: translatedName, attr: "iceResist", type: "FLAT", value: def.resistances.ice * 100, category: "equipment" });
                        if (statName === "poisonResist" && def.resistances.poison) modifiers.push({ source: translatedName, attr: "poisonResist", type: "FLAT", value: def.resistances.poison * 100, category: "equipment" });
                        if (statName === "energyResist" && def.resistances.energy) modifiers.push({ source: translatedName, attr: "energyResist", type: "FLAT", value: def.resistances.energy * 100, category: "equipment" });
                        if (statName === "physicalResist" && def.resistances.physical) modifiers.push({ source: translatedName, attr: "physicalResist", type: "FLAT", value: def.resistances.physical * 100, category: "equipment" });
                    }
                }
            }
        });
    ...
    public calculateWeaponAttack(
        ...
        // 2. Skill Bonus
        const level = overrideLevel !== undefined ? overrideLevel : state.getLevel();
        const str = state.getStrengthLevel(); 
     ...
     */
    public getAllNonZeroStats(state: PlayerState): { id: string; result: StatResult }[] {
        // List of all possible stats we care about
        // Expand this list as needed.
        const allStats = [
            "maxHealth", "maxMana", 
            "attack", "defense", "armor", "speed", 
            "capacity", "memory", 
            "criticalChance", "criticalDamage",
            "strength", "dexterity", "intelligence", "reflex", // Base stats
            "range", "cooldown", "lightRadius",
            "fireResist", "iceResist", "poisonResist", "energyResist", "physicalResist"
        ];
    ...
    public getOffensiveExpBonus(state: PlayerState): StatResult {
        ...
        const offensiveSlots: EquipmentSlot[] = [EquipmentSlot.MAIN_HAND, EquipmentSlot.RING, EquipmentSlot.NECK];
        ...
        const totalFlat = modifiers.filter(m => m.category !== "base").reduce((acc, m) => acc + m.value, 0);
    ...
    public getDefensiveExpBonus(state: PlayerState): StatResult {
        ...
        const defensiveSlots: EquipmentSlot[] = 
            [EquipmentSlot.OFF_HAND, EquipmentSlot.HEAD, EquipmentSlot.BODY, EquipmentSlot.LEGS, EquipmentSlot.BOOTS];
    ...
    public calculateDPSBreakdown(state: PlayerState) {
        ...
        const dps = ((1 - (critChance / 100)) * avgNormalDmg + (critChance / 100) * avgCritDmg) * aps;
    ...
    public calculateAPSBreakdown(state: PlayerState) {
        ...
        return {
            ...
            modifiers: cooldownRes.breakdown.sources.filter(m => m.category !== "base")
        };
    }
    ...
    public calculateItemDPS(item: any, state: PlayerState): number {
        const def = WeaponRegistry.getWeaponDefinition(item.itemId);
        if (!def || (def.type !== ItemType.MELEE && def.type !== ItemType.SWORD && 
                     def.type !== ItemType.AXE && def.type !== ItemType.CLUB && 
                     def.type !== ItemType.RANGED && def.type !== ItemType.DISTANCE)) {
            return 0;
        }
        ...
        const critChanceRes = this.calculateStat("criticalChance", state, overrides);
