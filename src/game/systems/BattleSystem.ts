...
export default class BattleSystem {
  ...
  private getPlayerElementResistance(element: string): number {
    ...
    const total = slots.reduce((acc, slot) => {
      ...
      const def = WeaponRegistry.getWeaponDefinition(equipped.itemId);
      if (!def) return acc;

      return acc + (def.resistances?.[element] ?? 0);
    }, 0);

    return Phaser.Math.Clamp(total, -0.95, 0.95);
  }
  ...
  private handleEnemyAttack(enemy: Enemy, player: Player): void {
    ...
    console.log(
      `[COMBAT:LOG] Enemy ${enemy.id} (${enemy.enemyType}) attacking player. Dist: ${distance.toFixed(1)}px, Range: ${attackRange}px`,
    );

    const attackDamage = enemy.getDamage();
    const playerDefense = player.getTotalDefense();

    // Use enemy definition/magic profile instead of a single hardcoded type.
    const isFire = this.isEnemyFireAttack(enemy);
    ...
    if (defenseRoll >= attackRoll) {
      if (isFire) {
        ...
        this.logBattle(
          "combat_partially_blocked_player",
    ...
    // Resistência Elemental do Jogador (vindo de todos os equips relevantes)
    if (isFire) {
      const fireResistance = this.getPlayerElementResistance("fire");
      damage = Math.round(damage * (1 - fireResistance));
    }
    ...
    if (damage > 0) {
      new FloatingText(
        this.scene,
        player.sprite.x,
        player.sprite.y - (damageMitigation > 0 ? 60 : 20),
        -damage,
        false,
        isFire ? "#FF4500" : undefined,
        isFire ? "🔥" : undefined,
      );
      this.logBattle(
        "combat_damage_taken",
  ...
  private handlePlayerAttack(player: Player, enemy: Enemy): void {
    ...
    if (isCritical) {
      ...
      // Log
      console.log(
        `[Combat] CRITICAL HIT! Chance: ${critChance.toFixed(1)}% Dmg: ${damage} (Range: ${minCrit}-${maxCrit})`,
      );
    ...
    if (damage > 0) {
      ...
      if (localStorage.getItem("tgs_settings_blood") !== "false") {
        ...
        const maxHp =
          EnemyRegistry.getEnemyDefinition(enemy.enemyType)?.health || 100;
  ...
  private gainCombatExperience(player: Player, damageDealt: number = 0): void {
    ...
    if (weapon?.element === "fire") {
      ...
      console.warn(
        "[Combat XP] Unknown weapon type, defaulting to Strength:",
        weapon.type,
        totalXp,
      );
      player.gainStrengthExperience(totalXp);
  ...
  public castRuneEffect(runeId: string, x: number, y: number): void {
    ...
    const hasLos = gameScene.mapLoader.checkLineOfSight(
      startX,
      startY,
      x,
      y,
    ...
    if (isStar) {
      ...
      if (closest) {
        ...
        targetPos = {
          x: (closest as Enemy).sprite.x,
          y: (closest as Enemy).sprite.y,
        };
      } else {
    ...
    const projectile = new RuneProjectile(
      ...
      (impactTarget: Enemy | null) => {
        this.applyRuneImpact(
          runeId,
          impactTarget,
          isStar ? null : targetPos,
          def,
          isStar,
        );
      },
    ...
    // Play Launch Sound
    if (isStar)
      AudioManager.getInstance().playStarHit(); // Launch sound? Using Hit for now
    else AudioManager.getInstance().playFireHit();
  }

  // Extracted Impact Logic
  private applyRuneImpact(
    runeId: string,
    hitEnemy: Enemy | null,
    ...
    if (isStar) {
      ...
      const damageRange = RuneRegistry.calculateDamage(
        runeId,
        this.player.getLevel(),
        this.player.getIntelligenceLevel(),
      );
      dmg = Phaser.Math.Between(damageRange.min, damageRange.max);
    }

    // Willpower Bonus
    const willpowerBonusPct = this.player.getWillpowerBonusPercent
      ? this.player.getWillpowerBonusPercent()
      : 0;
      ...
      if (localStorage.getItem("tgs_settings_blood") !== "false") {
        ...
        if (isStar && wasDead) {
          ...
          this.bloodSystem.emitBlood(
            ...
            finalDamage,
            maxHp,
            isNaN(enemy.getBloodColor()) ? 0xff0000 : enemy.getBloodColor(),
          );
        }
      }
