...
export class MapLoader {
  ...
  public async loadAllLevels(mapName: string): Promise<void> {
    try {
      ...
      console.log(
        `[BMS] Sub-systems initialized for ${this.binaryLevels.size} levels.`,
      );
    } catch (error) {
  ...
  public checkLineOfSight(
    ...
    for (const point of points) {
      ...
      if (symbol && symbol !== "...") {
        const tileDef =
          this.mapMetadata.tileDefinitions[symbol] ||
          this.mapMetadata.entityTemplates[symbol];
        if (tileDef) {
          const tileId =
            tileDef.id ||
            (tileDef.under
              ? this.resolveSymbolToId(tileDef.under, this.mapMetadata)
              : null);
  ...
  public seedMapItemsToPersistence(mapName: string, level: string): void {
    ...
    if (!data || !data.levelData) {
      ...
      console.warn(
        `[MapLoader] Cannot seed items. Data missing for ${levelKey}`,
      );
      return;
    }
    ...
    items.forEach((item) => {
      ...
      if (item.contents && item.contents.length > 0) {
        item.contents.forEach((content) => {
          ...
          console.log(
            `[MapLoader] Seeding Content: ${content.id} (Count: ${content.count}). Found Def: ${!!def}, Stackable: ${isStackable}`,
          );
