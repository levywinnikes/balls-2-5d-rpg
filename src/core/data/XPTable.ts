...
export class XPTable {
  ...
  static getLevel(xp: number): { level: number; xpRequired: number } {
  ...
  static getLevelProgress(xp: number): {
    ...
    if (currentLevel >= this.getMaxLevel()) {
      return {
        ...
        nextLevelXp: current.xpRequired,
        progress: 1,
      };
    }

    const next = this.levels[currentLevel];
    const prev = this.levels[currentLevel - 1] || { level: 0, xpRequired: 0 };
    const xpIntoLevel = xp - prev.xpRequired;
    const xpForNext = next.xpRequired - prev.xpRequired;
    const progress = clamp(xpIntoLevel / xpForNext, 0, 1);
  ...
  static getXpForLevel(level: number): number {
    ...
    return found ? found.xpRequired : this.levels[this.levels.length - 1].xpRequired;
  }
  ...
  static getLevelInfo(currentXP: number): {
    ...
    if (currentLevel >= this.getMaxLevel()) {
      ...
      return {
        level: last.level,
        currentLevelXP: last.xpRequired,
        nextLevelXP: Infinity,
        progress: 1,
      };
    }

    const next = this.levels.find((l) => l.level === currentLevel + 1);
    const nextLevelXP = next ? next.xpRequired : Infinity;
    const currentLevelXP = current.xpRequired;
    const progress =
      nextLevelXP === Infinity
        ? 1
        : clamp((currentXP - currentLevelXP) / (nextLevelXP - currentLevelXP), 0, 1);
