...
export class NavigationService {
  ...
  public static init(mapData: any, binaryLevels: Record<string, Uint8Array>) {
    ...
    this.worker = new Worker(
      new URL("../workers/navigation.worker.ts", import.meta.url),
    );
    ...
    this.worker.onmessage = (e) => {
      ...
      if (type === "MAP_READY") {
        ...
        console.log(
          "[NavigationService] Path found with",
          data.path.length,
          "points.",
        );
        ...
        console.warn(
          "[NavigationService] Path calculation failed: location unreachable.",
        );
        const playerState = PlayerState.getInstance();
        playerState.setActiveRoute(null);
        playerState.emit("uiNotification", {
          type: "warning",
          message: t_game("msg_destination_unreachable"),
        });
      }
    };
  ...
  public static findPath(
    startGrid: { x: number; y: number; level: string },
    endGrid: { x: number; y: number; level: string },
  ) {
