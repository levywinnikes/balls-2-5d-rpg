...
export class EditorScene extends Phaser.Scene {
  ...
  private handlePointerMove(pointer: Phaser.Input.Pointer) {
    ...
    if (this.isDragging) {
      if (pointer.primaryDown) {
        ...
        (pointer.isDown && this.spaceKey && this.spaceKey.isDown)
      ) {
        // Pan
  ...
  private paintTile(worldX: number, worldY: number) {
    ...
    if (this.selectedTool === "eraser") {
      ...
      const entry = Object.entries(mapData.tileDefinitions).find(
        ([sym, def]: any) => def.id === this.selectedTile,
      );
      if (entry) {
        ...
        let counter = 1;
        while (
          mapData.tileDefinitions[attempt] ||
          mapData.entityTemplates?.[attempt]
        ) {
        ...
        // Add definition
        console.log(
          `[Editor] Creating new symbol '${attempt}' for tile '${this.selectedTile}'`,
        );
