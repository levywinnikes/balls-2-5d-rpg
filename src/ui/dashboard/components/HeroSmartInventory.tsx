...
import { StarIcon, getItemStars, getItemBorder, TIER_BG } from "../../utils/ItemVisuals";
...
const InventorySlot: React.FC<{
    ...
    return (
        ...
        >
             {/* Rarity BG Glow */}
            <div className={`absolute inset-0 ${tier.bg || TIER_BG.common} opacity-10 group-hover:opacity-20 transition-opacity`} />
                ...
                <>
                    ...
                    />
                     ...
                     {(() => {
                        ...
                        if (def && (def.consumable || def.type === ItemType.FOOD || def.type === ItemType.RESOURCE || def.type === ItemType.POTION)) {
                             return (
...
}> = React.memo(({ activeSection, selectedSlot, focusedItemIndex, onEquip, items, onItemClick, onHoverItem, onLeaveItem, filterMode = 'all' }) => {
                ...
                <div className="grid grid-cols-5 gap-3">
                    {filteredItems.map((item, idx) => (
                        <InventorySlot 
                            ...
                            item={item} 
                            index={idx}
                            isFocused={idx === focusedItemIndex && activeSection === "INVENTORY"}
                            onClick={onItemClick}
                            onHover={onHoverItem}
                            onLeave={onLeaveItem}
                        />
