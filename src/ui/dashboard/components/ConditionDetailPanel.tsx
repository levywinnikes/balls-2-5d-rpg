...
export const ConditionDetailPanel: React.FC<ConditionDetailPanelProps> = ({
  ...
  // Core Attributes
  if (
    ["strength", "dexterity", "intelligence", "reflex"].includes(conditionType)
  ) {
    ...
    > = {
      strength: {
        ...
        icon: (
          ...
          >
            <path
              d="M14.5 4L17 6.5L9 14.5C8.5 15 8.5 16 9 16.5L10.5 18C11 18.5 12 18.5 12.5 18L20.5 10L23 12.5L13 22.5C11.5 24 9.5 23.5 8.5 22.5L4.5 18.5C3.5 17.5 3 15.5 4.5 14L14.5 4Z"
              fill="currentColor"
            />
            <path
              d="M4 2L6.5 4.5L2.5 8.5L2 4C2 3 3 2 4 2Z"
              fill="currentColor"
              fillOpacity="0.5"
            />
          </svg>
      ...
      dexterity: {
        ...
        bonuses: [
          {
            label: "Bônus de Ataque (Distância)",
            value: (level) => `+${level * 5}%`,
          },
          {
            label: "Chance Crítica",
            value: (level) => `+${(level * 0.2).toFixed(1)}%`,
          },
        ],
      },
      intelligence: {
    ...
    const statData =
      conditionType === "strength"
        ? ps.getStrengthData()
        : conditionType === "dexterity"
          ? ps.getDexterityData()
          : conditionType === "intelligence"
            ? ps.getIntelligenceData()
            : ps.getReflexData();

    // Calculate XP Progress
    const nextLevelXP =
      conditionType === "strength"
        ? ps.getStrengthNextLevelExp()
        : conditionType === "dexterity"
          ? ps.getDexterityNextLevelExp()
          : conditionType === "intelligence"
            ? ps.getIntelligenceNextLevelExp()
            : ps.getReflexNextLevelExp();

    // Progress percentage for the bar
    const progress = Math.min(
      100,
      Math.max(0, (statData.experience / nextLevelXP) * 100),
    );
    ...
    return (
      <div
        className={`h-full flex flex-col bg-black/60 rounded-xl p-4 border ${colors.border}`}
      >
        {/* Header */}
        ...
        <div
          className={`text-center py-6 bg-gradient-to-r ${colors.bg} to-transparent rounded-lg border ${colors.border} mb-4 relative overflow-hidden group shadow-lg ${colors.glow}`}
        >
          <div
            className={`absolute inset-0 ${colors.bgLight} opacity-0 group-hover:opacity-100 transition-opacity`}
          />
          <div
            className={`text-[10px] ${colors.textLight} opacity-60 uppercase tracking-widest mb-2`}
          >
            Nível Atual
          </div>
          <div
            className={`text-6xl font-black ${colors.text} drop-shadow-2xl scale-110`}
          >
            {statData.level}
          </div>
          ...
          <div className="mt-3 flex flex-col items-center gap-2">
            {attr.bonuses.map((bonus, idx) => (
              <div
                key={idx}
                className={`px-3 py-1 rounded-full bg-black/40 border ${colors.border} backdrop-blur-sm`}
              >
                <div
                  className={`text-xs ${colors.textLight} font-mono flex items-center gap-1`}
                >
                  <span>{bonus.label}:</span>
                  <span className="font-bold text-white">
                    {bonus.value(statData.level)}
                  </span>
                </div>
              </div>
            ))}
        ...
        {/* XP Progress */}
        <div
          className={`bg-black/40 rounded-lg p-3 border ${colors.border} mb-4 shadow-inner`}
        >
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <div className="space-y-3">
            <div>
              <div
                className={`text-xs ${colors.textLight} opacity-80 font-bold uppercase tracking-wider mb-2`}
              >
                Descrição
              </div>
            ...
            <div>
              ...
              <div className="grid gap-2 text-xs">
                {attr.affects.map((effect, idx) => (
                  <div
                    key={idx}
                    className={`${colors.bgLight} p-2.5 rounded border ${colors.border} flex items-center gap-2`}
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${colors.bg.replace("bg-", "bg-").replace("/10", "")}`}
                    />
                    {effect}
                  </div>
                ))}
            ...
            <div
              className={`${colors.bg} border ${colors.border} rounded-lg p-3 flex gap-3 items-start`}
            >
              <div className="mt-0.5">💡</div>
              <div>
                <div className={`text-xs ${colors.text} font-bold mb-0.5`}>
                  Dica
                </div>
                <div
                  className={`text-xs ${colors.textLight} opacity-80 leading-snug`}
                >
                  Ganhe XP de {attr.title} combatendo inimigos e usando itens
                  apropriados!
                </div>
              </div>
            </div>
    ...
    return (
      <div className="h-full flex flex-col bg-black/60 rounded-xl p-4 border border-yellow-500/20">
        ...
        <div className="text-center py-6 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 rounded-lg border border-yellow-500/20 mb-4">
          <div className="text-[10px] text-yellow-400/60 uppercase tracking-widest mb-2">
            Nível Atual
          </div>
        ...
        <div className="bg-black/40 rounded-lg p-3 border border-yellow-500/10 mb-4">
          ...
          <div className="text-xs text-white/60 font-mono text-right">
            {Math.floor(charExp)} /{" "}
            {charNextLevel === Infinity ? "MAX" : charNextLevel} XP
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="text-sm text-white/70 leading-relaxed">
                O nível representa a experiência geral do seu personagem. Cada
                nível aumenta seus atributos base.
              </div>
            </div>
              ...
              <div className="grid gap-2 text-xs">
                <div className="bg-red-500/10 p-2 rounded border border-red-500/20">
                  ...
                  <div className="text-white/60">
                    +5 por nível (a partir do nível 2)
                  </div>
                </div>
            ...
            <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
              ...
              <div className="text-xs text-green-300/80">
                Complete missões e derrote inimigos para ganhar XP e subir de
                nível!
              </div>
            </div>
          </div>
    ...
    return (
      <div className="h-full flex flex-col bg-black/60 rounded-xl p-4 border border-purple-500/20">
        ...
        <div className="text-center py-6 bg-purple-500/10 rounded-lg border border-purple-500/20 mb-4">
          <div className="text-[10px] text-purple-400/60 uppercase tracking-widest mb-2">
            Tier Atual
          </div>
          <div className="text-5xl font-bold text-purple-300">{wpTier}</div>
          <div className="text-sm text-purple-400/80 mt-2">
            +{wpBonus}% Stats
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-purple-500/10 p-2 rounded border border-purple-500/10">
                  PV Máximo
                </div>
                <div className="bg-purple-500/10 p-2 rounded border border-purple-500/10">
                  Mana Máxima
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="space-y-2 text-xs">
              <div
                className={`p-2 rounded border ${hungerTier === 5 ? "bg-orange-500/20 border-orange-500/40" : "bg-orange-500/5 border-orange-500/10"}`}
              >
              ...
              <div
                className={`p-2 rounded border ${hungerTier === 4 ? "bg-orange-500/20 border-orange-500/40" : "bg-orange-500/5 border-orange-500/10"}`}
              >
                <div className="font-bold text-orange-300">Tier 4 (70-89%)</div>
                <div className="text-white/60">75% Regeneração</div>
              </div>
              <div
                className={`p-2 rounded border ${hungerTier === 3 ? "bg-orange-500/20 border-orange-500/40" : "bg-orange-500/5 border-orange-500/10"}`}
              >
                <div className="font-bold text-orange-300">Tier 3 (50-69%)</div>
                <div className="text-white/60">50% Regeneração</div>
              </div>
              <div
                className={`p-2 rounded border ${hungerTier === 2 ? "bg-orange-500/20 border-orange-500/40" : "bg-orange-500/5 border-orange-500/10"}`}
              >
                <div className="font-bold text-orange-300">Tier 2 (30-49%)</div>
                <div className="text-white/60">25% Regeneração</div>
              </div>
              <div
                className={`p-2 rounded border ${hungerTier === 1 ? "bg-orange-500/20 border-orange-500/40" : "bg-orange-500/5 border-orange-500/10"}`}
              >
                <div className="font-bold text-orange-300">Tier 1 (10-29%)</div>
                <div className="text-white/60">10% Regeneração</div>
              </div>
              <div
                className={`p-2 rounded border ${hungerTier === 0 ? "bg-red-500/20 border-red-500/40" : "bg-red-500/5 border-red-500/10"}`}
              >
