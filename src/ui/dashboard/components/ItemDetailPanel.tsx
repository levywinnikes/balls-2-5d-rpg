...
export const ItemDetailPanel: React.FC<ItemDetailPanelProps> = ({
  ...
  const rawAttributes =
    item.attributes && item.attributes.length > 0 ? item.attributes : [];
    ...
    <div className="h-full flex flex-col bg-black/60 rounded-xl p-4 border border-white/10 relative overflow-hidden">
      ...
      <div className="flex gap-4 border-b border-white/10 pb-4 mb-4">
        <div className="w-16 h-16 rounded bg-black/50 border border-white/20 flex items-center justify-center relative shadow-inner">
          ...
          />
          {item.count > 1 && (
            <span className="absolute bottom-1 right-1 text-xs font-bold text-white drop-shadow">
              {item.count}
            </span>
          )}
        </div>
      ...
      {def && (def as any).damage > 0 && (
        <div className="mb-4 p-4 bg-blue-500/10 rounded-xl border border-blue-500/20 text-center">
          <div className="text-[10px] text-blue-300/60 uppercase tracking-widest mb-1 font-bold">
          ...
          <div className="text-[9px] text-white/20 mt-1 uppercase italic">
            Considerando seus atributos atuais
          </div>
        </div>
      )}
          ...
          <div className="space-y-1">
            ...
            {attributesList.map((attr: any, idx: number) => {
              ...
              return (
                <div
                  key={idx}
                  className={`flex items-center gap-2 text-xs text-white/80 group`}
                >
                  <StarIcon
                    quality={quality}
                    size={10}
