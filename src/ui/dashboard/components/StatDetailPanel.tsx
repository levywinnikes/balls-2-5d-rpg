...
export const StatDetailPanel: React.FC<StatDetailPanelProps> = ({ statKey, label }) => {
    const { t } = useLanguage();
    ...
    const enhancedModifiers = result.breakdown.sources.map(mod => {
        if (mod.type === "PERCENT") {
            const absoluteContribution = parseFloat(((subtotal * mod.value) / 100).toFixed(2));
                ...
                {enhancedModifiers.length > 0 ? (
                    <div className="space-y-2">
                        ...
                        {enhancedModifiers.map((mod: any, i: number) => {
                            ...
                            const categoryTranslated = t(`stat_category_${mod.category}` as any) || mod.category;
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-col">
                                        <span className="text-white/80 font-medium">{mod.source}</span>
                                        <span className="text-[9px] text-white/40 uppercase">{categoryTranslated}</span>
                                    </div>
                                    <div className={`font-bold ${mod.value > 0 ? 'text-green-400' : 'text-red-400'}`}>
                                        {absoluteText && <span className="text-white/60 mr-1">{absoluteText}</span>}
                ...
                {result.breakdown.globalMultipliers && result.breakdown.globalMultipliers.length > 0 && (
                    <div className="space-y-2 mt-3">
                        ...
                        {result.breakdown.globalMultipliers.map((mod: any, i: number) => {
                            ...
                            const absoluteContribution = parseFloat((afterPercent * (mod.value - 1)).toFixed(2));
                            const categoryTranslated = t(`stat_category_${mod.category}` as any) || mod.category;
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-col">
                                        <span className="text-purple-300 font-medium">{mod.source}</span>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {absoluteContribution > 0 && (
                                            <span className="text-white/60 text-sm">({absoluteContribution})</span>
                                        )}
