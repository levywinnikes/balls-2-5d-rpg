...
export const StarPointsDetailPanel: React.FC = () => {
  ...
  const getTierBg = (tier: string) => {
    switch (tier) {
      case "bronze":
        return "bg-amber-600/15 border-amber-500/30";
      case "silver":
        return "bg-slate-300/15 border-slate-400/30";
      case "gold":
        return "bg-yellow-500/15 border-yellow-400/30";
      default:
                ...
                <div className="flex flex-wrap gap-1">
                  {item.stars.map((star, j) => (
                    ...
                    >
                      <Star
                        size={8}
                        className={`${getTierColor(star.tier)} fill-current`}
                      />
                      <span className={getTierColor(star.tier)}>
                        {getTierLabel(star.tier)}
                      </span>
