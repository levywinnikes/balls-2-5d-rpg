...
import { EquipmentSlot, SLOT_COMPATIBILITY, ItemType } from "../../config/ItemConstants";
...
export const HeroDashboard: React.FC = () => {
    ...
    const inventory = usePlayerState("inventoryUpdated", () => [...PlayerState.getInstance().getInventory()], []);
    const equipment = usePlayerState("equipmentChanged", () => {
        const ps = PlayerState.getInstance();
        return {
    ...
    const filteredItems = useMemo(() => {
        ...
        if (selectedSlot === 'consumables') {
            return inventory.filter((item: any) => {
                ...
                if (!def) return false;
                const type = def.type;
                return type === ItemType.FOOD || type === ItemType.RUNE || type === ItemType.POTION;
            });
        }
    ...
    // Get selected equipped item (when equipment slot is clicked)
    const selectedEquippedItem = selectedSlot && selectedSlot !== 'all' && selectedSlot !== 'consumables' ? (equipment as any)[selectedSlot as EquipmentSlot] : null;
    
    // Left Panel State (Item Details vs Stat Breakdown)
    const [leftPanelMode, setLeftPanelMode] = useState<'item' | 'stat' | 'condition'>('item');
    const [selectedStat, setSelectedStat] = useState<{ key: string; label: string } | null>(null);
    const [selectedCondition, setSelectedCondition] = useState<'willpower' | 'hunger' | 'strength' | 'dexterity' | 'intelligence' | 'reflex' | 'characterLevel' | null>(null);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            const keys = ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Enter", " ", "Escape", "Tab", "q", "e"];
            if (!keys.includes(e.key.toLowerCase()) && !keys.includes(e.key)) return;
            
            // Tab / Q / E Handling
            if (e.key === "Tab" || e.key.toLowerCase() === "q" || e.key.toLowerCase() === "e") {
                e.preventDefault();
                e.stopPropagation();
            ...
            if (activeSection === "EQUIPMENT") {
                ...
                if (e.key === "Enter" || e.key === " ") {
                     ...
                     if (filteredItems[focusedItemIndex]) {
                         const globalIndex = inventory.findIndex((i: any) => i.uid === filteredItems[focusedItemIndex].uid);
                         if (globalIndex !== -1) {
    ...
    }, [isOpen, activeTab, activeSection, filteredItems, focusedItemIndex, selectedSlot, setFocusedSlotIndex, closeWindow, handleEquipmentNav, handleInventoryNav, handleTabSwitch, inventory]); 
    ...
    // Optimized Handlers
    const handleSlotClick = useCallback((slotEnum: EquipmentSlot) => {
    ...
    const handleStatSelect = useCallback((statKey: string, label: string) => {
        ...
        const coreAttributes = ['strength', 'dexterity', 'intelligence', 'reflex', 'characterLevel'];
        if (coreAttributes.includes(statKey)) {
            setSelectedCondition(statKey as 'strength' | 'dexterity' | 'intelligence' | 'reflex' | 'characterLevel');
    ...
    const handleConditionClick = useCallback((conditionType: 'willpower' | 'hunger') => {
    ...
    useEffect(() => {
        if (isOpen) {
             ...
             const ps = PlayerState.getInstance();
             if (ps.getInventory().length > 0 || Object.keys(ps.getEquipment()).length > 0) {
            ...
            >
                ...
                <div className="grid grid-cols-12 h-full">
                    ...
                    <div className="col-span-4 border-r border-white/10 bg-black/40 h-full overflow-hidden relative">
                         ...
                         {leftPanelMode === 'condition' && selectedCondition ? (
                         ...
                         ) : leftPanelMode === 'stat' && selectedStat?.key === 'starPoints' ? (
                            ...
                            (() => {
                                ...
                                const targetItem = hoveredItem || 
                                                  (activeSection === 'EQUIPMENT' ? selectedEquippedItem : focusedInventoryItem) || 
                                ...
                                return targetItem ? (
                                    <ItemDetailPanel 
                                        item={targetItem} 
                                        isEquipped={!!selectedEquippedItem && targetItem === selectedEquippedItem}
                                        onLight={(() => {
                                            return (targetItem.itemId === 'torch') ? () => PlayerState.getInstance().lightTorch() : undefined;
                                        })()}
                                        onExtinguish={(() => {
                                            return (targetItem.itemId === 'light_torch') ? () => PlayerState.getInstance().extinguishTorch() : undefined;
                                        })()}
                                        onUse={() => {
                                             if(!targetItem) return;
                                             const globalIndex = inventory.findIndex((i: any) => i.uid === targetItem.uid);
                                             if (globalIndex !== -1) {
                                                  PlayerState.getInstance().useInventoryItem(globalIndex);
                                             }
                                        }}
                                        ...
                                        onUnequip={() => {
                                             if (selectedSlot && selectedSlot !== 'all' && selectedSlot !== 'consumables') {
                                        ...
                                        onDrop={() => {
                                             if(!targetItem) return;
                                             // Drop using item UID
                                             PlayerState.getInstance().dropItem(targetItem.uid, targetItem.count || 1);
                                        }}
                                        isEquippable={true}
                                    />
                                ) : (
                            ...
                            <div className="h-full flex items-center justify-center text-white/20 text-xs uppercase tracking-widest p-8 text-center">
                                {t("character_overview" as any) || "Clique em um atributo para ver detalhes"}
                            </div>
                         )}
                    </div>
                    ...
                    <div className="col-span-8 flex flex-col h-full bg-black/10">
                        ...
                        <div className="h-16 border-b border-white/10 flex items-center justify-center px-6 bg-white/5 relative z-[110] gap-8">
                                ...
                                */}
                                ...
                                >
                                    <div className={`p-1.5 rounded ${activeTab === 'equipment' ? 'bg-yellow-400/10' : 'bg-white/5'}`}>
                                        <Sword size={16} />
                                    </div>
                                ...
                                >
                                    <div className={`p-1.5 rounded ${activeTab === 'stats' ? 'bg-yellow-400/10' : 'bg-white/5'}`}>
                                        <Shield size={16} />
                                    </div>
                        ...
                        <div className="flex-1 overflow-hidden relative">
                            ...
                            {activeTab === "stats" && (
                                <div className="h-full overflow-y-auto p-6 custom-scrollbar">
                                    <HeroStatsTab onStatSelect={handleStatSelect} onConditionClick={handleConditionClick} />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="h-10 border-t border-white/10 bg-black/40 flex items-center justify-center gap-6 text-[10px] text-white/40 uppercase tracking-widest shrink-0">
                            <span className="flex items-center gap-1"><kbd className="bg-white/10 px-1 rounded">TAB</kbd> {t("dashboard.hint_switch_tab" as any) || "Switch Tab"}</span>
                            <span className="flex items-center gap-1"><kbd className="bg-white/10 px-1 rounded">Arrows</kbd> {t("dashboard.hint_nav" as any) || "Navigate"}</span>
                            <span className="flex items-center gap-1"><kbd className="bg-white/10 px-1 rounded">Enter</kbd> {t("dashboard.hint_select" as any) || "Select"}</span>
                            <span className="flex items-center gap-1"><kbd className="bg-white/10 px-1 rounded">Esc</kbd> {t("dashboard.hint_back" as any) || "Back"}</span>
                        </div>
                    </div>
                </div>
            </div>
