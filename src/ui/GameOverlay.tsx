...
export const GameOverlay: React.FC = () => {
    ...
    const { openWindow, closeWindow, isWindowOpen, openWindows } = useWindowSystem();
    const [contextMenu, setContextMenu] = useState<{x: number, y: number, options: ContextMenuOption[], data: any} | null>(null);
    ...
    useEffect(() => {
        ...
        // Detect removals
        const removedIds = Object.keys(prevOpenWindows.current).filter(id => !openWindows[id]);
    ...
    useEffect(() => {
        ...
        const shouldPause = windows.heroMenu || windows.settings || windows.systemMenu || windows.cheats;
    ...
    const handleQuickSave = async () => {
        ...
        if(scene.showFloatingText && scene.player?.sprite) {
             scene.showFloatingText(scene.player.sprite.x, scene.player.sprite.y - 50, "Quick Saved!", 0x00ff00);
        }
    };
        ...
        if((window as any).electronAPI) {
             ...
             if(data) {
                  scene.scene.restart(data);
                   if(scene.showFloatingText && scene.player?.sprite) {
                     setTimeout(() => scene.showFloatingText(scene.player.sprite.x, scene.player.sprite.y - 50, "Quick Loaded!", 0xffff00), 100);
                  }
             }
        } else {
            alert("Quick Load only available in Native Mode (or from memory session).");
        }
    };
    ...
    }, [windows, windowPositions, isEditorMode, toggleWindow, handleQuickSave, handleQuickLoad]); 
    ...
    useEffect(() => {
      ...
      const handleDrop = (e: DragEvent) => {
          if (draggedItem) {
              ...
              const isEquipment = draggedItem.source === "equipment" || (draggedItem.uid && draggedItem.uid.startsWith("equipped_"));
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
          ...
          const cancelKeys = ["escape", "arrowup", "arrowdown", "arrowleft", "arrowright", "w", "a", "s", "d"];
    ...
    useEffect(() => {
      ...
      const handleRequest = (req: any) => {
          ...
          if (req.type === "ground_item") {
              ...
              if (item.count > 1) options.push({ label: `Pickup All (${item.count})`, action: "pickup_all" });
          }
          if (options.length > 0) {
      ...
      return () => { PlayerState.getInstance().off("requestContextMenu", handleRequest); };
    }, []);
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 60 }}>
            ...
            <div className="contents pointer-events-auto">
                 <SystemMenuUI isOpen={windows.systemMenu} onClose={() => toggleWindow("systemMenu")} />
                 <DialogueWindow />
            </div>
        </div>
