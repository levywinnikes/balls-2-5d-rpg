...
import { TileRegistry, TileDefinition } from "../../game/graphics/tiles/TileRegistry";
...
export const MapEditorUI: React.FC = () => {
  ...
  const [entities, setEntities] = useState<string[]>(["rat", "spider", "skeleton", "goblin", "orc", "rotworm", "giant_spider", "demon", "dragon", "chest"]); // Basic list for now
      ...
      <div className="w-64 bg-stone-900 border-r-2 border-stone-600 flex flex-col pointer-events-auto h-full overflow-hidden">
        ...
        <div className="p-2 grid grid-cols-2 gap-2 bg-stone-800 border-b border-stone-700">
             ...
             <div className="col-span-2 flex items-center justify-between bg-stone-900 p-1 rounded">
                 <button onClick={() => handleLevelChange(-1)} className="px-2 bg-stone-700 text-white rounded">-</button>
        ...
        <div className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-stone-600">
            {/* TILES */}
            <div className="mb-4">
                <h3 className="text-stone-400 text-xs font-bold mb-2 uppercase border-b border-stone-700 pb-1">Tiles</h3>
            ...
            {/* ENTITIES / SPECIAL */}
            <div className="mb-4">
                <h3 className="text-stone-400 text-xs font-bold mb-2 uppercase border-b border-stone-700 pb-1">Enemies/Items</h3>
                ...
                <div className="text-[10px] text-gray-500 mt-2 italic">
                    Note: To verify Entities, the map loader must recognize their IDs. For newMap.json, entities are mapped by short-codes.
                    The current editor implementation simplifies painting by putting the full ID in the map for now.
                    The MapLoader needs to be robust enough to handle full IDs or we need a mapping step.
                </div>
            </div>
