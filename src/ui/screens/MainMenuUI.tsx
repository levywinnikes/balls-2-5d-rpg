...
import { Sword, Scroll, X, Trash2, Droplets, User } from "lucide-react";
import { AudioManager } from "../../game/systems/AudioManager";
// Native API Type Definition
interface Window {
  electronAPI?: {
    saveGame: (name: string, data: any) => Promise<any>;
    loadGame: (name: string) => Promise<any>;
    listSaves: () => Promise<any>;
    deleteGame: (name: string) => Promise<any>;
  };
}
...
export const MainMenuUI: React.FC<MainMenuUIProps> = ({ onStart }) => {
  ...
  const { setScale, s, windows, setAllWindows, toggleEditorMode } = useUI();
  ...
  const handleDelete = async (name: string) => {
      if(window.confirm(t("menu_delete_confirm").replace("{name}", name))) {
          if (window.electronAPI?.deleteGame) {
              await window.electronAPI.deleteGame(name);
  ...
  const btnStyle = "w-64 py-3 bg-[#222] hover:bg-[#333] border-2 border-[#444] rounded-lg text-[#fbbf24] font-bold text-lg flex items-center justify-center gap-3 transition-all transform hover:scale-105 shadow-lg active:scale-95";
  const inputStyle = "w-64 p-3 bg-[#111] border-2 border-[#444] rounded text-white text-center focus:border-[#fbbf24] outline-none mb-4";
      ...
      <div className="relative z-10 flex flex-col items-center animate-fade-in">
        ...
        {menu === "new" && (
          <div className="flex flex-col items-center bg-[#1a1a1a] p-8 rounded-xl border border-[#333] shadow-2xl">
             <h2 className="text-2xl text-white mb-6 font-bold">{t("menu_create_character")}</h2>
             <input 
                ...
                maxLength={12}
             />
             <button onClick={handleCreate} className={`${btnStyle} mb-4 bg-[#2a2a2a]`}>
                {t("menu_start")}
             </button>
             <button onClick={() => setMenu("main")} className="text-gray-500 hover:text-white underline text-sm mt-2">
                {t("menu_back")}
             </button>
          </div>
        )}
        ...
        {menu === "load" && (
           <div className="flex flex-col items-center w-[90vw] max-w-2xl h-[70vh] bg-[#1a1a1a] p-6 rounded-xl border border-[#333] shadow-2xl relative">
              <h2 className="text-2xl text-white mb-6 font-bold">{t("menu_load_game")}</h2>
              
              <div className="w-full flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-3 pr-2">
                 {saves.length === 0 && <div className="text-gray-500 text-center italic mt-10">{t("menu_no_saves")}</div>}
                 
                 {saves.map((save, i) => (
                     <div key={i} className="bg-[#222] p-4 rounded border border-[#333] hover:border-[#555] flex items-center justify-between group transition-all">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-[#333] rounded-full flex items-center justify-center text-[#fbbf24]">
                                <User size={24} />
                            </div>
                            <div>
                                <div className="text-[#fbbf24] font-bold text-lg">{save.playerState?.characterName || "Unknown"} <span className="text-gray-400 text-sm ml-2">(Lvl {save.playerState?.level || 1})</span></div>
                                <div className="text-gray-500 text-xs">
                                   {t("menu_play_time")}: {Math.floor((save.playerState?.playTime || 0)/3600)}h {Math.floor(((save.playerState?.playTime || 0)%3600)/60)}m 
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button 
                                onClick={() => handleDelete(save.playerState?.characterName)}
              ...
              <button onClick={() => setMenu("main")} className="absolute top-4 right-4 text-gray-500 hover:text-white">
                 ✖ {t("menu_back")}
              </button>
           </div>
        )}
