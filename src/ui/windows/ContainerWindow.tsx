...
export const ContainerContent: React.FC = () => {
    const { s, showTooltip, hideTooltip, setDraggedItem, draggedItem, groundDrag } = useUI();
    const { t } = useLanguage();
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if(draggedItem.source === "container" && draggedItem.containerId === containerId) {
            return;
        }
    ...
    const containerRegistryDef = defId ? ContainerRegistry.getContainer(defId) : null;
    const capacity = containerRegistryDef ? containerRegistryDef.maxSlots : 20;
    
    // Highlight logic
    const isHighlighting = !!groundDrag || (draggedItem && (draggedItem.source !== "container" || draggedItem.containerId !== containerId));
    const highlightBorder = isHighlighting ? "1px dashed #00d2ff" : "1px solid #444";
             ...
             <div 
                ...
                onMouseUpCapture={(e) => {
                    if (groundDrag && containerId) {
                         ...
                         if(ps.addItemToContainer(containerId, groundDrag.item.weaponId, groundDrag.item.count || 1, groundDrag.item.itemId)) {
                  ...
                  {items.map((item) => {
                      ...
                      return (
                          <RPGSlot
                            ...
                            onMouseEnter={(e) => {
                                if (def && !groundDrag) {
                                    const { name, subtext } = formatItemTooltip(def, { weaponId: item.itemId, uid: item.uid });
                                    showTooltip({
                                        text: name,
                                        subtext,
                                        x: e.clientX, 
                            ...
                            onClick={() => {
                                ...
                                if (playerState.hasCapacity && !playerState.hasCapacity(itemWeight)) {
                                    playerState.emit("message", t_game("msg_too_heavy"));
                                    return;
                                }
                                if (playerState.addItem(item.itemId, item.count)) {
                                    if(containerId) playerState.removeItemFromContainer(containerId, item.uid);
                                } else {
                            ...
                            onContextMenu={(e) => {
                                e.preventDefault();
                                if (containerId && PlayerState.getInstance().toggleContainerItem(containerId, item.uid)) {
