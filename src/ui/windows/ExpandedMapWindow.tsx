...
import { ChevronUp, ChevronDown, Plus, Minus, MapPin, Trash2, Maximize, LocateFixed } from "lucide-react";
        ...
        <div ref={windowRef} style={{ display: "flex", flexDirection: "column", height: "100%", position: "relative" }}>
            ...
            >
                <div style={{ display: "flex", gap: "2px" }}>
                    <button onClick={() => setZoom((z) => Math.min(z + 0.5, 4))} style={btnStyle} title="Zoom In">
                        <Plus size={14 * scale} />
                    </button>
                    <button onClick={() => setZoom((z) => Math.max(z - 0.5, 0.1))} style={btnStyle} title="Zoom Out">
                        <Minus size={14 * scale} />
                    </button>
                    <button onClick={handleFitToScreen} style={{ ...btnStyle, color: "#a855f7" }} title="Fit to Screen">
                        <Maximize size={14 * scale} />
                    </button>
                    <button onClick={handleCenter} style={{ ...btnStyle, color: "#3b82f6" }} title="Center on Player">
            ...
            {menu && (
                ...
                >
                    <div onClick={handleAddMarker} style={{ padding: "8px 12px", cursor: "pointer", color: "#ddd", display: "flex", alignItems: "center", gap: "8px" }} className="map-menu-item">
                        <Plus size={14} /> Add Marker
                    </div>
                    {selectedMarkerId && (
                        <>
                            <div onClick={handleRenameMarker} style={{ padding: "8px 12px", cursor: "pointer", color: "#fbbf24", display: "flex", alignItems: "center", gap: "8px", borderTop: "1px solid #444" }}>
                                <MapPin size={14} /> Rename Marker
                            </div>
                            {renamingMarkerId === selectedMarkerId && (
                                <div style={{ padding: "4px 8px", borderTop: "1px solid #444", background: "rgba(0,0,0,0.3)" }}>
                                    <input 
                                        autoFocus
                                        value={renamingValue}
                                        onChange={(e) => setRenamingValue(e.target.value)}
                                        onKeyDown={(e) => e.key === "Enter" && handleConfirmRename()}
                                        onBlur={() => renamingValue === "" && setRenamingMarkerId(null)}
                                        style={{ width: "100%", background: "#444", border: "1px solid #666", color: "white", padding: "4px", borderRadius: "4px", fontSize: "12px", outline: "none" }}
                                    />
                                    <div style={{ display: "flex", gap: "4px", marginTop: "4px" }}>
                                        <button onClick={handleConfirmRename} style={{ flex: 1, fontSize: "10px", padding: "2px", background: "#3b82f6", color: "white", border: "none", borderRadius: "2px" }}>Save</button>
                                        <button onClick={() => setRenamingMarkerId(null)} style={{ flex: 1, fontSize: "10px", padding: "2px", background: "#555", color: "white", border: "none", borderRadius: "2px" }}>Cancel</button>
                                    </div>
                                </div>
                            )}
                            <div onClick={() => handleRemoveMarker(selectedMarkerId as string)} style={{ padding: "8px 12px", cursor: "pointer", color: "#f87171", display: "flex", alignItems: "center", gap: "8px", borderTop: "1px solid #444" }}>
