...
export const SystemMenuUI: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  isOpen,
  onClose,
}) => {
  const { s, t, windows, windowPositions, toggleWindow } = useUI();
  ...
  const handleSaveToFile = async () => {
      ...
      if(!saveSystem) {
          alert("Error: Save System not found. Check console.");
          return;
      }
          ...
          try {
              ...
              if(success) {
                   ...
                   if(scene.showFloatingText && scene.player?.sprite) {
                       scene.showFloatingText(scene.player.sprite.x, scene.player.sprite.y - 50, t("msg_game_saved"), 0x00ff00);
                   }
  ...
  const handleExitToTitle = async () => {
      ...
      if(!saveSystem) {
           alert("Error: Save System not found. Cannot save before exit.");
           return;
      }
      ...
      if(scene && saveSystem) {
          ...
          try {
            ...
            window.location.reload();
              ...
              alert("Error during Save & Exit: " + e);
          }
      }
  };
    ...
    <div style={{
      ...
      <div style={{ 
          ...
          backdropFilter: 'blur(12px)'
            ...
            <button className="system-btn-large" onClick={handleExitToTitle} style={{...btnStyle(s), background: '#622', borderColor: '#844'}}>
                <LogOut size={24} /> 
                 <div style={{display:'flex', flexDirection:'column', alignItems:'flex-start'}}>
...
const btnStyle = (s: (n: number) => number) => ({
    ...
    background: 'var(--bg-glass-light)', border: '1px solid var(--border-subtle)', color: 'var(--text-primary)',
    padding: s(15), borderRadius: 8, cursor: 'pointer',
    fontSize: s(16), justifyContent: 'flex-start',
    transition: 'all 0.2s',
    width: '100%',
