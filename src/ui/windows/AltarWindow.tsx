...
export const AltarContent: React.FC = () => {
  ...
  const [selectedRuneId, setSelectedRuneId] = useState<string>(
    RuneRegistry.getAllRunes()[0]?.id || "",
  );
  const [storedRunes, setStoredRunes] = useState<
    Array<{ runeId: string; count: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    if (
      ps.currentOpenedContainerId &&
      ps.currentOpenedContainerDefId === "altar" &&
      !altarId
    ) {
  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      ...
      const count = inv.reduce(
        (acc, item) => (item.itemId === "magic_rune" ? acc + item.count : acc),
        0,
      );
  ...
  const handleTransferConfirm = () => {
    ...
    if (transferRequest.direction === "store") {
      ...
      if (
        ps.withdrawRuneFromAltar(altarId, transferRequest.itemId, transferQty)
      ) {
    ...
    if (!itemData) {
      try {
        ...
        if (raw) {
          ...
          if (p.runeId && p.source === "spellbook")
            itemData = { itemId: p.runeId, source: "spellbook" };
        }
      } catch (e) {}
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <img
              src={`assets/items/magic_rune.png`}
              alt={t("item_magic_rune" as any)}
              style={{
                width: "100%",
                height: "100%",
                imageRendering: "pixelated",
                opacity: blankRunesCount > 0 ? 1 : 0.5,
              }}
            />
            <span
              style={{
        ...
        >
          <div
            style={{ fontSize: "11px", fontWeight: "bold", color: "#d8b4fe" }}
          >
            {t("ui_craft" as any)}
          </div>
            ...
            >
              {RuneRegistry.getAllRunes().map((r) => (
                <option key={r.id} value={r.id}>
                  {r.name} ({r.memoryCost} {t("memory_short" as any)})
                </option>
              ))}
            </select>
            <button
              ...
              style={{
                padding: "4px 12px",
                backgroundColor: blankRunesCount > 0 ? "#7e22ce" : "#444",
                color: blankRunesCount > 0 ? "#fff" : "#888",
                border: "none",
                borderRadius: "4px",
                cursor: blankRunesCount > 0 ? "pointer" : "default",
                fontWeight: "bold",
                fontSize: "11px",
              }}
            >
        ...
        >
          <div
            style={{
              ...
              height: "100%",
              backgroundColor:
                memoryCurrent > memoryMax ? "#ef4444" : "#a855f7",
      ...
      {transferRequest && (
        ...
        >
          ...
          >
            {transferRequest.direction === "store"
              ? t("ui_store_qty_title" as any)
              : t("ui_withdraw_qty_title" as any)}
          </span>
          <input
            type="number"
            min="1"
