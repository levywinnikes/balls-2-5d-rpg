...
import { QuestManager, QuestDefinition, ActiveQuestState } from "../../game/systems/QuestManager";
...
export const QuestLogContent: React.FC = () => {
  ...
  const [questDefs, setQuestDefs] = useState<Map<string, QuestDefinition>>(new Map());
  ...
  useEffect(() => {
    ...
    return () => { ps.off("questUpdated", updateData); };
  ...
  const getCurrentList = () => {
      ...
      return completedQuests.map(id => ({ questId: id, stageId: 999, conditionsProgress: [] }));
  };

  const selectedQuestDef = selectedQuestId ? questDefs.get(selectedQuestId) : null;
  const selectedQuestState = activeQuests.find(q => q.questId === selectedQuestId);
                ...
                {getCurrentList().map((qState) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {activeTab === "active" && (
                                <div className="text-xs text-gray-500 truncate">{def.type.toUpperCase()}</div>
                            )}
                        </div>
                    );
                })}
             ...
             {selectedQuestDef ? (
                 <div className="space-y-4">
                     ...
                     <div>
                         ...
                         <p className="text-gray-300 italic">{selectedQuestDef.description}</p>
                     </div>
                     ...
                     {activeTab === "active" && selectedQuestState && (
                         <div className="space-y-3">
                             ...
                             <div className="bg-black/30 p-3 rounded border border-white/5 space-y-2">
                                 {(() => {
                                     const stage = selectedQuestDef.stages.find(s => s.id === selectedQuestState.stageId);
                                         ...
                                         <>
                                            ...
                                            <div className="space-y-2">
                                                {stage.conditions.map((cond, idx) => {
                                                    const current = selectedQuestState.conditionsProgress[idx] || 0;
                                                    ...
                                                    return (
                                                        <div key={idx} className="flex items-center gap-2 text-sm">
                                                            ...
                                                            <span className={isDone ? "text-gray-500 line-through" : "text-gray-300"}>
                                                                {cond.type === "kill" && `Kill ${cond.target} (${current}/${target})`}
                                                                {cond.type === "collect" && `Collect ${cond.target} (${current}/${target})`}
                                                                {cond.type === "talk" && `Talk to ${cond.target}`}
                                                            </span>
                                                        </div>
                                                    );
                                                })}
                     ...
                     {activeTab === "completed" && (
                         <div className="bg-green-900/20 border border-green-500/30 p-4 rounded text-center">
                             ...
                             <h3 className="text-green-400 font-bold">{t("quest_completed") || "Quest Completed"}</h3>
                         </div>
                     )}
