...
import { PlayerState, SkillState } from "../../game/entities/Player/PlayerState";
...
import { Sword, Shield, Crosshair, Star, Zap, Utensils, Brain } from "lucide-react";
...
export const SidebarSkills: React.FC = () => {
    ...
    // Hooks for data
    const levelData = usePlayerState(["experienceChanged", "levelUp", "reset"], () => ({
        level: playerState.getLevel(),
        percent: XPTable.getLevelInfo(playerState.getExperience()).progress * 100
    }), { level: 1, percent: 0 });

    const strengthData = usePlayerState("strengthExperienceChanged", () => ({
        level: playerState.getStrengthLevel(),
        percent: StrengthXpTable.getLevelInfo(playerState.getStrengthExperience()).progress * 100
    }), { level: 1, percent: 0 });

    const dexterityData = usePlayerState("dexterityExperienceChanged", () => ({
        level: playerState.getDexterityLevel(),
        percent: DexterityXpTable.getLevelInfo(playerState.getDexterityExperience()).progress * 100
    }), { level: 1, percent: 0 });

    const reflexData = usePlayerState("reflexExperienceChanged", () => ({
        level: playerState.getReflexLevel(),
        percent: ReflexXpTable.getLevelInfo(playerState.getReflexExperience()).progress * 100
    }), { level: 1, percent: 0 });

    const intelligenceData = usePlayerState("intelligenceExperienceChanged", () => ({
        level: playerState.getIntelligenceLevel(),
        percent: IntelligenceXpTable.getLevelInfo(playerState.getIntelligenceExperience()).progress * 100
    }), { level: 1, percent: 0 });

    const stats = {
      atk: usePlayerState(["equipmentChanged", "strengthExperienceChanged", "dexterityExperienceChanged", "levelUp"], () => playerState.getTotalAttack(), 10),
      def: usePlayerState(["equipmentChanged", "reflexExperienceChanged", "levelUp"], () => playerState.getTotalDefense(), 0),
      armor: usePlayerState(["equipmentChanged", "levelUp"], () => playerState.getTotalArmor(), 0),
      speed: usePlayerState(["equipmentChanged", "willpowerUpdated", "levelUp"], () => playerState.getCurrentSpeed(), 100),
      capacity: usePlayerState(["levelUp"], () => playerState.getCapacity(), 400),
      currentWeight: usePlayerState("inventoryUpdated", () => playerState.getCurrentWeight(), 0),
    };

    const willpowerData = usePlayerState(["willpowerUpdated", "reset", "levelUp"], () => ({
      tier: playerState.getWillpowerTier(),
      unlocked: playerState.getLevel() >= 2
    }), { tier: 0, unlocked: false });

    const hunger = usePlayerState(["hungerUpdated", "reset"], () => playerState.getHunger(), 1000);
        ...
        <div className="flex flex-col p-2 h-full overflow-y-auto custom-scrollbar">
            ...
            {/* Combat Totals with Detailed Tooltips */}
             <div className="grid grid-cols-2 gap-y-2 gap-x-1 text-[10px] text-[#aaa] mb-2">
                     ...
                     onMouseEnter={(e) => {
                        ...
                        const tooltipContent = (
                             <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-0.5 text-[10px]">
                                ...
                                <div className="col-span-2 h-px bg-gray-600 my-0.5"/>
                                <span className={`font-bold ${isMagic ? "text-purple-400" : "text-orange-400"}`}>{t("tooltip_total")}:</span>
                                <span className={`font-bold ${isMagic ? "text-purple-400" : "text-orange-400"} text-right`}>{stats.atk}</span>
                            </div>
                        );
                     ...
                     onMouseEnter={(e) => {
                        ...
                        const tooltipContent = (
                             <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-0.5 text-[10px]">
                                ...
                                <div className="col-span-2 h-px bg-gray-600 my-0.5"/>
                                <span className="font-bold text-green-400">{t("tooltip_total")}:</span>
                     ...
                     onMouseEnter={(e) => {
                         ...
                         const tooltipContent = (
                            <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-0.5 text-[10px]">
                                ...
                                <div className="col-span-2 h-px bg-gray-600 my-0.5"/>
                                <span className="font-bold text-slate-300">{t("tooltip_total")}:</span>
                                <span className="font-bold text-slate-300 text-right">{stats.armor}</span>
                            </div>
                        );
                        showTooltip({
                            text: t("armor"),
                ...
                <span className="font-bold text-slate-400 text-right">{stats.armor}</span>
                     ...
                     onMouseEnter={(e) => {
                        ...
                        const tooltipContent = (
                            <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-0.5 text-[10px]">
                                ...
                                <span className="font-bold text-yellow-400 text-right">{stats.speed}</span>
                            </div>
                        );
                        showTooltip({
                            text: t("speed"),
            ...
            <div className="flex flex-col gap-2 mb-2">
                 ...
                 <div className="flex flex-col gap-1">
                     <div className="flex items-center justify-between text-[10px] text-[#aaa]">
                        ...
                        <span className="text-yellow-500 font-bold">{Math.min(100, Math.floor(Math.min(1000, hunger) / 10))} / 100</span>
                     </div>
                        ...
                        {[...Array(10)].map((_, i) => {
                            ...
                            return (
                                <div 
                                    ...
                                    onMouseLeave={hideTooltip}
                                >
                                    <div style={{ width: `${fillPct}%` }} className="h-full bg-yellow-500 transition-all duration-300" />
                                </div>
                            );
                        })}
                     </div>
                     {/* Stock Bar (1001-2000) */}
                     {hunger > 1000 && (
                        <div className="w-full h-1 bg-[#222] border border-[#333] rounded overflow-hidden"
                            onMouseEnter={(e) => {
                                showTooltip({
                                    text: t("hunger_stock"),
                ...
                {willpowerData.unlocked && (
                     ...
                     >
                         <div className="flex items-center justify-between text-[10px] text-[#aaa]">
                            ...
                            <span className="text-purple-500 font-bold">{t("rank")} {willpowerData.tier}</span>
                         </div>
                            ...
                            {[...Array(10)].map((_, i) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        onMouseLeave={hideTooltip}
                                    >
                                        <div style={{ width: `${fillPct}%` }} className="h-full bg-purple-500 transition-all duration-300" />
                                    </div>
                                );
                            })}
                         </div>
