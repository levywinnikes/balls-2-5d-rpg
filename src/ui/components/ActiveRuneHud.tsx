...
export const ActiveRuneHud: React.FC = () => {
  const { draggedItem, setDraggedItem, showTooltip, hideTooltip, windows } =
    useUI();
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (
        activeRuneId &&
        !currentRunes.find((r: any) => r.runeId === activeRuneId)
      ) {
  ...
  const handleCast = (runeId: string) => {
    if (isOnCooldown) {
      PlayerState.getInstance().emit(
        "message",
        t_game("msg_rune_cooldown_active"),
      );
      return;
    }
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (source === "altar_storage" && containerId && runeId) {
      if (
        PlayerState.getInstance().withdrawRuneFromAltar(containerId, runeId, 1)
      ) {
    ...
    >
      ...
      <div className="w-full">
        <div className="flex justify-between text-[8px] text-gray-400 mb-0.5 font-mono font-bold">
          ...
          <span
            className={
              isOverloaded ? "text-red-500 animate-pulse" : "text-purple-300"
            }
          >
        ...
        <div className="h-1.5 bg-[#1a1a1a] rounded-sm overflow-hidden border border-white/10 shadow-inner">
          <div
            ...
            style={{
              width: `${Math.min(100, (memoryUsage / Math.max(1, memoryCapacity)) * 100)}%`,
            }}
          />
        </div>
      </div>
