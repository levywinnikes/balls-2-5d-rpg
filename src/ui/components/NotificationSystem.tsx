...
import { PlayerState, NotificationType } from "../../game/entities/Player/PlayerState";
...
const NotificationItem: React.FC<{ notif: GameNotification }> = ({ notif }) => {
    ...
    const getStyles = (type: NotificationType) => {
        switch (type) {
            case "exp": return "border-yellow-600/60 bg-black/80 text-yellow-100";
            case "willpower": return "border-purple-500/50 bg-black/80 text-purple-300 font-bold";
            case "error": return "border-red-500/50 bg-red-950/80 text-red-100";
            case "warning": return "border-orange-500/50 bg-black/80 text-orange-200";
            case "pickup": return "border-white/10 bg-black/80 text-gray-200";
            case "heal": return "border-green-500/40 bg-black/80 text-green-300";
            default: return "border-gray-500/20 bg-black/80 text-gray-300";
        }
    };
    ...
    return (
        ...
        >
            ...
            <span className="relative z-10 text-xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">{getIcon(notif.type)}</span>
...
export const NotificationSystem: React.FC = () => {
    ...
    useEffect(() => {
        if (notifications.length > maxNotifications) {
             ...
             if (activeNotifs.length > maxNotifications) {
                 ...
                 setNotifications(prev => prev.map(n => n.id === oldestId ? { ...n, closing: true } : n));
             }
        }
    }, [notifications]);
    ...
    useEffect(() => {
        const interval = setInterval(() => {
             ...
             setNotifications(prev => {
                 ...
                 return prev.map(n => !n.closing && now - n.timestamp > 4500 ? { ...n, closing: true } : n);
             });
        }, 500);
    ...
    useEffect(() => {
        ...
        const handleUiNotification = (data: { 
            ...
            addNotification(data.type, data.message, data.startProgress, data.endProgress);
        };

        const handleWillpower = (tier: number) => {
            addNotification("willpower", t_game("notif_reached_tier").replace("{tier}", tier.toString()));
        };
