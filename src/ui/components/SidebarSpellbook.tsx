...
export const SidebarSpellbook: React.FC = () => {
    ...
    // Transfer State
    const [transferRequest, setTransferRequest] = useState<{ containerId: string; runeId: string } | null>(null);
            ...
            <div className="mb-2">
                ...
                <div className="h-1.5 bg-[#222] rounded-full overflow-hidden">
                    <div 
                        ...
                        style={{ width: `${Math.min(100, (memoryUsage / memoryCapacity) * 100)}%` }}
                    />
                </div>
            </div>
    ...
    // title={`${def.name}\nCharges: ${rune.count}\nMem: ${def.memoryCost}`} // Replaced by rich tooltip
                            ...
                            onMouseEnter={(e) => {
                                const mockDef = { 
                                 ...
                                 const { name, subtext } = formatItemTooltip(mockDef, { weaponId: rune.runeId, itemId: rune.runeId, count: rune.count });
                ...
                {/* Empty Slots Filler */}
                {Array.from({ length: Math.max(0, totalSlots - runes.length) }).map((_, i) => (
                    <div 
                        key={`empty-${i}`} 
