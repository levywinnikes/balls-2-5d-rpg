...
export const HeroInventory = React.memo(
  ({ items, balance }: HeroInventoryProps) => {
    const { setDraggedItem, showTooltip, hideTooltip, draggedItem } = useUI();
    const { t } = useLanguage();
    ...
    const handleDragStart = (e: React.DragEvent, item: InventoryItem) => {
      ...
      e.dataTransfer.setData(
        "text/plain",
        JSON.stringify({ uid: item.uid, source: "inventory" }),
      );
      const img = new Image();
      img.src =
    ...
    return (
      ...
      >
        <div className="flex justify-between items-center px-1 mb-1 border-b border-[var(--border-subtle)] pb-1">
          ...
          <div className="flex gap-1">
            <button
              ...
              className={`p-1 rounded transition-colors ${viewMode === "grid" ? "bg-[var(--bg-glass-hover)] text-[var(--accent-gold)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
            >
              <Grid size={12} />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-1 rounded transition-colors ${viewMode === "list" ? "bg-[var(--bg-glass-hover)] text-[var(--accent-gold)]" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
            >
              <List size={12} />
            </button>
          </div>
        ...
        <div
          className="flex-1 custom-scrollbar overflow-y-auto p-1 bg-black/20 rounded inner-shadow"
