...
import { Star, Shield, Zap, Sword, Heart, Activity, Brain, Box, Wind, Crosshair, Sparkles, Beef } from "lucide-react";
...
export const HeroStatsTab: React.FC<HeroStatsTabProps> = ({ onStatSelect, onConditionClick }) => {
    const ps = PlayerState.getInstance();
    const { t } = useLanguage();

    // --- Survival Hooks ---
    const hp = usePlayerState(["healthChanged", "levelUp", "equipmentChanged"], () => ps.getHealth(), 100);
    const maxHp = usePlayerState(["levelUp", "equipmentChanged", "buffsChanged"], () => ps.getMaxHealth(), 100);
    
    // Hunger (Max 2000 strict as per source)
    const hunger = usePlayerState(["hungerUpdated", "reset"], () => ps.getHunger(), 1000);
    const maxHunger = 2000; // Defined in PlayerState source

    // --- Core Stats with XP ---
    const strData = usePlayerState("strengthExperienceChanged", () => {
        const d = ps.getStrengthData();
        return { ...StrengthXpTable.getLevelInfo(d.experience), totalXP: d.experience };
    }, { level: 1, currentLevelXP: 0, nextLevelXP: 100, progress: 0, totalXP: 0 });

    const dexData = usePlayerState("dexterityExperienceChanged", () => {
        const d = ps.getDexterityData();
        return { ...DexterityXpTable.getLevelInfo(d.experience), totalXP: d.experience };
    }, { level: 1, currentLevelXP: 0, nextLevelXP: 100, progress: 0, totalXP: 0 });

    const intData = usePlayerState("intelligenceExperienceChanged", () => {
        const d = ps.getIntelligenceData();
        return { ...IntelligenceXpTable.getLevelInfo(d.experience), totalXP: d.experience };
    }, { level: 1, currentLevelXP: 0, nextLevelXP: 100, progress: 0, totalXP: 0 });

    const refData = usePlayerState("reflexExperienceChanged", () => {
        const d = ps.getReflexData();
        return { ...ReflexXpTable.getLevelInfo(d.experience), totalXP: d.experience };
    }, { level: 1, currentLevelXP: 0, nextLevelXP: 100, progress: 0, totalXP: 0 });

    // --- Willpower ---
    const wpExp = usePlayerState("willpowerUpdated", () => ps.getWillpowerExp(), 0);
    const wpTarget = usePlayerState("willpowerUpdated", () => ps.getWillpowerTarget(), 300);
    const wpTier = usePlayerState("willpowerUpdated", () => ps.getWillpowerTier(), 0);
    const wpBonus = usePlayerState("willpowerUpdated", () => ps.getWillpowerBonusPercent(), 0);

    // --- Derived Stats ---
    // We fetch these once or on equipment changes
    const derivedStatsVersion = usePlayerState(["equipmentChanged", "buffsChanged", "levelUp"], () => Date.now(), 0);
    ...
    const renderCoreCard = (label: string, statKey: string, data: { level: number, progress: number, currentLevelXP: number, nextLevelXP: number, totalXP: number }, icon: React.ReactNode, color: string) => {
        // XP display values
        const xpInLevel = data.totalXP - data.currentLevelXP;
        const xpForNextLevel = data.nextLevelXP - data.currentLevelXP;
        const displayTarget = data.nextLevelXP === Infinity ? "MAX" : xpForNextLevel;
        ...
        >
            ...
            <div className="flex flex-col relative z-10 h-full justify-between">
                <div>
                    <span className="text-[10px] uppercase text-white/50 tracking-widest font-bold block">{label}</span>
                    <span className={`text-2xl font-bold ${color} leading-none my-1 block`}>{data.level}</span>
                </div>
                
                {/* XP Bar */}
                <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden mt-2 relative">
                    {(() => {
                        const percent = data.nextLevelXP === Infinity ? 100 : (data.progress * 100);
    ...
    const renderDetailRow = (statKey: string, label: string, value: string | number, icon: React.ReactNode) => (
        <div 
    ...
    return (
        <div className="h-full flex flex-col p-1 gap-2 overflow-y-auto custom-scrollbar relative" onClick={(e) => e.stopPropagation()}>
            ...
            >
                ...
                <div className="flex items-center justify-between mb-2 relative z-10">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <div className="text-[10px] text-yellow-400/60 uppercase tracking-widest">Nível do Personagem</div>
                            <div className="text-xl font-bold text-yellow-300">{ps.getLevel()}</div>
                        </div>
                    </div>
                    <div className="text-xs text-yellow-400/60 font-mono">
                        {Math.floor(ps.getExperience())} / {Math.ceil(XPTable.getXPRequiredForLevel(ps.getLevel() + 1))} XP
                    </div>
                </div>
                ...
                >
                   ...
                   <div className="absolute inset-0 flex items-center justify-between px-3 z-10">
                        <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-orange-100 shadow-black drop-shadow-md">
                             <Beef size={10} className="fill-orange-500 text-orange-500"/> Hunger
                        </span>
                        <span className="text-xs font-bold text-white shadow-black drop-shadow-md">
                            {Math.floor(hunger)}
                        </span>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
                {renderCoreCard(t("strength"), "strength", strData, <Sword />, "text-red-500")}
                {renderCoreCard(t("dexterity"), "dexterity", dexData, <Crosshair />, "text-green-400")}
                {renderCoreCard(t("intelligence"), "intelligence", intData, <Brain />, "text-purple-400")}
                {renderCoreCard(t("reflex"), "reflex", refData, <Wind />, "text-blue-400")}
            </div>
                ...
                <div className="relative z-10 flex flex-col gap-2">
                    <div className="flex justify-between items-center bg-black/20 p-1 rounded">
                         <span className="flex items-center gap-2 text-[11px] font-bold uppercase text-purple-200 tracking-widest pl-1">
                             <Sparkles size={12} className="text-purple-400" /> {t("willpower") || "Willpower"}
                         </span>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-white/50 font-mono px-1">
                        <span>XP: {Math.floor(wpExp)} / {wpTarget}</span>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
                {renderDetailRow("dps", t("stats.dps" as any) || "DPS", dpsData.totalDPS, <Sword className="text-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]"/>)}
                {renderDetailRow("cooldown", t("stats.cooldown" as any) || "Attack Speed", `${dpsData.aps} APS`, <Zap className="text-yellow-400"/>)}
                {renderDetailRow("attack", t("attack"), derived.attack, <Sword className="text-red-400"/>)}
                {renderDetailRow("defense", t("defense"), derived.defense, <Shield className="text-blue-400"/>)}
                {renderDetailRow("armor", t("armor"), derived.armor, <Shield className="text-slate-400"/>)}
                {renderDetailRow("criticalChance", "Crit %", `${derived.critChance}%`, <Activity className="text-yellow-400"/>)}
                {renderDetailRow("criticalDamage", "Crit Dmg", `${derived.critDmg}%`, <Activity className="text-orange-400"/>)}
                {renderDetailRow("speed", t("speed"), derived.speed, <Wind className="text-cyan-400"/>)}
                {renderDetailRow("capacity", "Weight", `${(ps.getCurrentWeight() / 100).toFixed(1)} / ${derived.capacity}`, <Box className="text-amber-700"/>)}
                {renderDetailRow("starPoints", t("star_points" as any) || "Star Points", starPoints, <Star className="text-yellow-400 fill-yellow-400"/>)}
            </div>
