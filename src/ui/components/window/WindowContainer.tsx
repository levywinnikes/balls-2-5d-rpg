...
export const WindowContainer: React.FC<WindowContainerProps> = ({ instance }) => {
    ...
    const currentSize = useRef(instance.size || { 
    ...
    return (
        <div
            ...
            style={{
                ...
                height: instance.minimized ? `${headerHeight}px` : `${currentSize.current.height * scale}px`
            }}
        >
            {/* --- HEADER --- */}
            <div
                ...
                onDoubleClick={() => updateWindow(instance.id, { minimized: !instance.minimized })}
                style={{
                    height: `${headerHeight}px`,
                    backgroundColor: "var(--bg-header)",
                    borderBottom: instance.minimized ? "none" : "1px solid var(--border-subtle)",
                    display: "flex",
                    alignItems: "center",
            ...
            >
                ...
                <div className="flex gap-1 items-center">
                    <button 
                         ...
                         onClick={() => updateWindow(instance.id, { pinned: !instance.pinned })}
                         title={instance.pinned ? "Unpin" : "Pin"}
                    >
                        {instance.pinned ? <Lock size={12 * scale} color="#fbbf24" /> : <Unlock size={12 * scale} color="#9ca3af" />}
                    </button>
                    <button 
                        className="hover:text-[var(--text-highlight)] transition-colors p-[2px]"
                        onClick={() => updateWindow(instance.id, { minimized: !instance.minimized })}
                    >
            ...
            {!instance.minimized && (
                 <div style={{ 
                     ...
                     <ContentFreezer frozen={isInteracting}>
                         {Content ? <Content windowId={instance.id} /> : <div>Content Not Found</div>}
                     </ContentFreezer>
                 </div>
            )}
