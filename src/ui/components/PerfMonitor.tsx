...
export const PerfMonitor: React.FC = () => {
  const [metrics, setMetrics] = useState(
    PlayerState.getInstance().getPerfData(),
  );
  ...
  useEffect(() => {
    const handlePerfUpdate = (data: any) => {
      ...
      s.totalUpdateTime =
        s.totalUpdateTime * (1 - alpha) + data.totalUpdateTime * alpha;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const debugKeysEnabled =
        PlayerState.getInstance().getDiagnosticSettings().enableDebugKeys;
      if (debugKeysEnabled) {
        if (e.key === "PageUp") {
    ...
    <div className="absolute top-10 right-2 z-[9000] flex flex-col gap-2 pointer-events-none select-none">
      ...
      <div className="bg-black/80 border border-emerald-500/50 p-3 rounded-lg shadow-2xl backdrop-blur-md min-w-[180px]">
        <div className="flex justify-between items-center border-b border-emerald-500/30 mb-2 pb-1">
          ...
          <span
            className={`text-[9px] font-bold ${metrics.totalUpdateTime > 14 ? "text-red-500 animate-pulse" : "text-emerald-500"}`}
          >
        ...
        <div className="space-y-1 text-[10px] font-mono">
            ...
            <div className="flex items-center gap-2">
              <div className="w-12 h-1 bg-gray-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (metrics.totalUpdateTime / 16.6) * 100)}%`,
                  }}
                />
              </div>
              <span
                className={
                  metrics.totalUpdateTime > 14
                    ? "text-red-400"
                    : "text-emerald-300"
                }
              >
          ...
          <div className="flex justify-between text-gray-500 text-[9px] pl-2 border-l border-gray-800">
            ...
            <span className="text-emerald-500/80">
              {formatMs(metrics.mapTime)}
            </span>
          </div>
          <div className="flex justify-between text-gray-500 text-[9px] pl-2 border-l border-gray-800">
            <span>Enemy AI</span>
            <span className="text-emerald-500/80">
              {formatMs(metrics.enemyTime)}
            </span>
          </div>
          <div className="flex justify-between text-gray-500 text-[9px] pl-2 border-l border-gray-800">
            <span>Physics</span>
            <span className="text-emerald-500/80">
              {formatMs(metrics.physicsTime)}
            </span>
          </div>
          ...
          <div className="flex justify-between border-t border-gray-800/50 mt-1 pt-1">
            ...
            <span
              className={`font-bold ${metrics.totalObjects > 5000 ? "text-orange-400" : "text-emerald-400"}`}
            >
          ...
          <div className="flex justify-between text-gray-500 text-[9px]">
            ...
            <span className="text-cyan-300 font-bold">
              {metrics.activeMeshes || 0}/{metrics.totalMeshes || 0}
            </span>
          </div>
          <div className="flex justify-between text-gray-500 text-[9px]">
            <span>Textures</span>
          ...
          <div className="flex justify-between text-gray-500 text-[9px]">
            ...
            <span className="text-purple-300 font-bold">
              {metrics.jsHeapUsedMb || 0}/{metrics.jsHeapTotalMb || 0}
            </span>
          </div>
          <div className="flex justify-between text-gray-500 text-[9px]">
            <span>Chunks (L/Q)</span>
            <span className="text-emerald-300 font-bold">
              {metrics.chunkLoaded || 0}/{metrics.chunkLoading || 0}
            </span>
          </div>
        </div>
      </div>
      ...
      {isDiagnosticOpen && (
        <div className="bg-slate-900/90 border border-blue-500/50 p-3 rounded-lg shadow-2xl backdrop-blur-md pointer-events-auto">
          ...
          <div className="space-y-1">
            {Object.entries(diag).map(([key, value]) => (
              ...
              >
                ...
                />
                <div
                  className={`w-3 h-3 rounded border transition-colors ${value ? "bg-blue-500 border-blue-400" : "bg-gray-800 border-gray-600"}`}
                >
                  {value && (
                ...
                <span
                  className={`text-[9px] font-mono transition-colors ${value ? (key.startsWith("hide") ? "text-red-400" : "text-gray-200") : "text-gray-500"}`}
                >
          ...
          <div className="mt-4 pt-2 border-t border-blue-500/20">
            <div className="text-[9px] text-blue-300 uppercase tracking-tighter mb-2 font-bold">
              Z-Axis Navigation
            </div>
          ...
          <div className="mt-4 pt-2 border-t border-blue-500/20">
            <div className="text-[9px] text-blue-300 uppercase tracking-tighter mb-1 font-bold">
              Object Breakdown
            </div>
            <div className="grid grid-cols-2 gap-x-2 gap-y-[1px] mb-2">
              {Object.entries(metrics.types || {}).map(
          ...
          <button
            onClick={() =>
              (window as any)._dynamicLevelRenderer?.purgeOrphans()
            }
