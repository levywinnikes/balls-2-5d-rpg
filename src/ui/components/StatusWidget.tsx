...
export const StatusWidget: React.FC = () => {
  ...
  const { showTooltip, hideTooltip, toggleWindow } = useUI();
  const { t } = useLanguage();

  // Data Hooks
  const currentHp = usePlayerState(
    "healthChanged",
  ...
  const currentXp = usePlayerState(
    "experienceChanged",
  ...
  const Bar = ({
    ...
    <div
      onClick={onClick}
      onMouseEnter={(e) =>
        tooltip && showTooltip({ ...tooltip, x: e.clientX, y: e.clientY })
      }
      ...
      <div className="flex items-center gap-2 h-5 pl-1">
        {showWeightAlert && (
          ...
          >
            <Backpack
              size={12}
              className={isOverburdened ? "text-red-500" : "text-yellow-500"}
            />
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-[2px] items-start">
        ...
        <div className="w-[320px]">
          <Bar
            ...
            tooltip={{
              text: t("hit_points"),
              subtext: "Click to open Hero Menu",
            }}
          />
        </div>
