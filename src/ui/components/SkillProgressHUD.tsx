...
export const SkillProgressHUD: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handleXpGain = (type: SkillType, data: { level: number, experience: number }, table: any, labelKey: string) => {
            ...
            setActiveSkills(prev => {
                ...
                if (existingIndex >= 0) {
                    ...
                    // If it was fading out but just received XP, restore it
                    const wasFadingOut = existing.isFadingOut;
        ...
        const onStrength = (data: any) => handleXpGain('strength', data, StrengthXpTable, 'stats_strength');
        const onDexterity = (data: any) => handleXpGain('dexterity', data, DexterityXpTable, 'stats_dexterity');
        const onReflex = (data: any) => handleXpGain('reflex', data, ReflexXpTable, 'stats_reflex');
        const onIntelligence = (data: any) => handleXpGain('intelligence', data, IntelligenceXpTable, 'stats_intelligence');
                ...
                return (
                    ...
                    >
                        <div className="flex justify-between items-center px-2 py-1.5 text-[10px] font-bold text-white uppercase tracking-wider">
                            <span className="text-white/80">{skill.label} <span className="text-white ml-1 text-xs">{skill.level}</span></span>
                            <span className={`${skill.colors.text} drop-shadow-md font-black`}>+{skill.accumulatedXp}</span>
                        </div>
