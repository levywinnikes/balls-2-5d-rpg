...
import {
  WeaponDefinition,
  WeaponRegistry,
} from "../../game/entities/weapons/WeaponRegistry";
...
const EquipSlot: React.FC<{
  ...
  // 2. Check UI Drag (Inventory OR Container)
    ...
    draggedItem &&
    (draggedItem.source === "inventory" || draggedItem.source === "container")
  ) {
    ...
    if (
      ...
      (def.type === expectedType ||
        (expectedType === "hand" &&
          (def.type === "melee" || def.type === "ranged")) ||
        (expectedType === "shield" &&
          (def.id === "torch" || def.id === "light_torch")))
    ) {
      isValidTarget = true;
    }
  }
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const targetId =
      draggedItem.source === "inventory" ? draggedItem.uid : draggedItem.itemId;
  ...
  return (
    ...
    >
      {item ? (
        ...
        <span
          className="text-[9px]"
          style={{ fontSize: size ? size * 0.25 : undefined }}
        >
          {label}
        </span>
      )}
...
export const EquipmentWidget: React.FC<{ customSlotSize?: number }> = ({
  customSlotSize,
}) => {
  ...
  const weaponItem = usePlayerState(
    ["weaponEquipped", "reset"],
    () => playerState.getEquippedItemObject("weapon"),
    null,
  );
  const helmetItem = usePlayerState(
    ["equipmentChanged", "reset"],
    () => playerState.getEquippedItemObject("helmet"),
    null,
  );
  const armorItemObj = usePlayerState(
    ["equipmentChanged", "reset"],
    () => playerState.getEquippedItemObject("armor"),
    null,
  );
  const legsItem = usePlayerState(
    ["equipmentChanged", "reset"],
    () => playerState.getEquippedItemObject("legs"),
    null,
  );
  const bootsItem = usePlayerState(
    ["equipmentChanged", "reset"],
    () => playerState.getEquippedItemObject("boots"),
    null,
  );
  const shieldItem = usePlayerState(
    ["equipmentChanged", "reset"],
    () => playerState.getEquippedItemObject("shield"),
    null,
  );

  // Helpers to get specific defs from objects if needed (EquipSlot usage might need updates)
  const getDef = (item: InventoryItem | null) =>
    item ? WeaponRegistry.getWeaponDefinition(item.itemId) || null : null;
  ...
  const handleDragStart = (
    ...
    e.dataTransfer.setData(
      "text/plain",
      JSON.stringify({ uid: mockInvItem.uid, source: "equipment" }),
    ); // Required for drag
      ...
      <div className="flex gap-2 items-center">
        <EquipSlot
          ...
          onClick={() => {
            if (
              weapon &&
              (weapon.id === "torch" || weapon.id === "light_torch")
            ) {
        ...
        <EquipSlot
          ...
          onDragStart={(e) =>
            armorItem && handleDragStart(e, armorItem, "armor")
          }
          onDragEnd={handleDragEnd}
        />
        <EquipSlot
