...
const MAP_NAME = "city_3d_mundi_p1";
...
function addP1Secrets(levels, blueprint, anchors) {
  const l0 = levels["0"];
  const lm1 = levels["-1"];
  const width = blueprint.mapSize.width;
  const height = blueprint.mapSize.height;

  // Urban: treasury room behind plaza
  {
    const x = anchors.urban.x - 18;
    const y = anchors.urban.y;
    room(l0, width, height, x - 5, y - 3, x + 5, y + 3, "bwl", "flr");
    fill(l0, width, height, x - 4, y - 2, x + 4, y - 1, "rok");
  }

  // Forest: hidden grotto with water
  {
    const x = anchors.forest.x + 12;
    const y = anchors.forest.y + 18;
    fill(l0, width, height, x - 4, y - 3, x + 4, y + 3, "wtr");
    room(lm1, width, height, x - 6, y - 5, x + 6, y + 5, "cwl", "cfl");
    fill(lm1, width, height, x - 3, y, x + 3, y, "wtr");
  }

  // Swamp: poison pools
  {
    const x = anchors.swamp.x + 8;
    const y = anchors.swamp.y - 8;
    fill(l0, width, height, x - 3, y - 3, x + 3, y + 3, "wtr");
    set(l0, width, height, x, y, "rok");
  }

  // Desert: buried chamber under sand
  {
    const x = anchors.desert.x - 16;
    const y = anchors.desert.y + 12;
    fill(l0, width, height, x - 6, y - 4, x + 6, y + 4, "snd");
    set(l0, width, height, x, y, "hol");
    room(lm1, width, height, x - 8, y - 6, x + 8, y + 6, "dwl", "dfn");
    set(lm1, width, height, x, y, "stu");
  }

  // Coast: shipwreck cellar
  {
    const x = anchors.coast.x - 24;
    const y = anchors.coast.y + 8;
    fill(l0, width, height, x, y - 2, x + 10, y + 2, "rok");
    set(l0, width, height, x + 5, y, "hol");
    room(lm1, width, height, x - 2, y - 8, x + 12, y + 6, "dwl", "dfn");
    set(lm1, width, height, x + 5, y, "stu");
  }

  // Highlands: watchtower vault
  {
    const x = anchors.high.x + 8;
    const y = anchors.high.y + 12;
    room(l0, width, height, x - 4, y - 3, x + 4, y + 3, "sdw", "flr");
    set(l0, width, height, x, y + 3, "hol");
    room(lm1, width, height, x - 6, y - 8, x + 6, y - 1, "cwl", "cfl");
    set(lm1, width, height, x, y + 3, "stu");
  }
}

function addP1LoreMarkers(levels, entities, blueprint, anchors) {
  entities["-2"].push(
    { symbol: "orc", x: anchors.desert.x - 6, y: anchors.desert.y - 18 },
    { symbol: "orc", x: anchors.desert.x - 14, y: anchors.desert.y - 18 },
  );

  entities["-1"].push(
    { symbol: "rat", x: anchors.coast.x - 22, y: anchors.coast.y + 5 },
    { symbol: "rat", x: anchors.coast.x - 26, y: anchors.coast.y + 5 },
    { symbol: "gob", x: anchors.swamp.x + 10, y: anchors.swamp.y - 6 },
  );

  entities["0"].push(
    { symbol: "gob", x: anchors.urban.x - 16, y: anchors.urban.y + 2 },
  );
}
...
function main() {
  ...
  addP1Secrets(levels, blueprint, anchors);
  addP1LoreMarkers(levels, entities, blueprint, anchors);
