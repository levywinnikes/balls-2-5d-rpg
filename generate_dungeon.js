...
function generateCave(
  ...
  for (let y = 0; y < height; y++) {
    map[y][0] = wallTile;
    map[y][width - 1] = wallTile;
  }
  for (let x = 0; x < width; x++) {
    map[0][x] = wallTile;
    map[height - 1][x] = wallTile;
  }
  ...
  if (enemies && enemies.length > 0) {
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        ...
        if (map[y][x] === floorTile && Math.random() < 0.03) {
          // 3% chance
...
const level_minus1 = generateCave(
  WIDTH,
  HEIGHT,
  WALL,
  FLOOR,
...
const level_minus2 = generateCave(
  WIDTH,
  HEIGHT,
  WALL,
  FLOOR,
...
const level_minus3 = generateCave(
  WIDTH,
  HEIGHT,
  WALL,
  FLOOR,
